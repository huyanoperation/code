         ...
         %xml.writeField( "key", "images" );
            ...
            %xml.writeClassBegin( "dict" );
                  ...
                  for( %i = 0; %i < %sceneGraph.getCount(); %i++ )
                  {
                        ...
                        else if( %sceneGraph.getObject(%i).getClassName() $= "t2dPath" )
                        {
                           if( %sceneGraph.getObject(%i).getName() $= ""  )
                           {
                              %xml.writeField( "key", "t2dPath_" @ "key" @ %count++ );
                           }
                           else
                           {
                              %xml.writeField( "key", "t2dPath_" @ %sceneGraph.getObject(%i).getName() );
                           }
                           %xml.writeClassBegin( "dict" );
                     
                           %xml.writeField( "key", "pathType" );
                           %xml.writeField("string", %sceneGraph.getObject(%i).pathType);
                        
                           %xml.writeField( "key", "nodeCount" );
                           %xml.writeField("string", %sceneGraph.getObject(%i).getNodeCount());
                           
                           %count = %sceneGraph.getObject(%i).getNodeCount();
                           for( %nodeCountIndex = 0;  %nodeCountIndex < %count; %nodeCountIndex++ )
                           {
                              %xml.writeField( "key", "node" @ %nodeCountIndex );
                              %xml.writeField("string", %sceneGraph.getObject(%i).getNode( %nodeCountIndex ) );
                           }
                           %xml.writeClassEnd();
                        }
                     }
